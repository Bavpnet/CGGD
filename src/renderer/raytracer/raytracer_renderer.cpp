...
void cg::renderer::ray_tracing_renderer::init()
{
	...
	lights.push_back({
			float3{-0.24f,  1.77f,  -0.22f},
			float3{0.78f, 0.78f, 0.78f}/4.f
	});
	lights.push_back({
			float3{0.23f,  1.77f,  -0.22f},
			float3{0.78f, 0.78f, 0.78f}/4.f
	});
	lights.push_back({
			float3{0.23f,  1.77f,   0.16f},
			float3{0.78f, 0.78f, 0.78f}/4.f
	});
...
void cg::renderer::ray_tracing_renderer::render()
{
	...
	raytracer->miss_shader = [](const ray& ray) {
		payload payload{};
		payload.color = {0.f, 0.f, 0.f}; //(ray.direction.y + 1.f) * 0.5f
		return payload;
	};
	...
	std::cout << "Raytrasing took " << duration.count() << " ms\n";
